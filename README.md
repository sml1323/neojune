...
./install_dev.sh
```
모든 프로그램이 하나의 도커 이미지에 포함된 버전입니다.  
테스트 환경과 관련된 모든 요소를 자동으로 설치하며,  
별도의 추가 설정 없이 손쉽게 개발 환경을 구축할 수 있도록 설계되었습니다.
...
./install_public.sh
```
도커 이미지를 다운로드하여 컨테이너로 실행합니다.  
클러스터링과 부하 분산 구성을 통해 안정적인 서비스 운영을 지원하며,  
실제 운영 환경에 적합한 모델입니다.
